 ...
 *   teardownWhenNoTests if set, global teardown will be set to run if no tests start
...
function NodeunitAsync(options) {
    ...
    this.teardownWhenNoTests = options.teardownWhenNoTests;
    this.noUncaughtExceptionHandler = options.noUncaughtExceptionHandler;
    ...
    if (this.fixtureTeardown && this.teardownWhenNoTests) {
        this._lastTeardownTimebomb = new Timebomb(this.fixtureTeardownDelayMs, this.fixtureTeardown);
    }

    // Have a listener for uncaught errors. Singleton as we only need one once we start testing
    if (!NodeunitAsync.listeningForUncaughtExceptions && !this.noUncaughtExceptionHandler) {
        NodeunitAsync.initUncaughtExceptionListener();
        NodeunitAsync.listeningForUncaughtExceptions = true;
    }
...
NodeunitAsync.listeningForUncaughtExceptions = false;
...
NodeunitAsync.prototype._endTest = function(error) {
    ...
    if (error && Array.isArray(this._currentTest._assertion_list)) {
      this._currentTest._assertion_list.reverse();
    }
